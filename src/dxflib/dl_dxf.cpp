...
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface,
    ...
    // Indicates start of new entity or var
        ...
        case DL_ENTITY_ELLIPSE:
            addEllipse2d(creationInterface);
            break;
...
void DL_Dxf::addArc(DL_CreationInterface* creationInterface) {
    DL_ArcData d(toReal(values[10]),
                 ...
                 toReal(values[30]),
                 toReal(values[40]),
                 toReal(values[50]) / 180.0 * M_PI,
                 toReal(values[51]) / 180.0 * M_PI);
 ...
 */
void DL_Dxf::addEllipse2d(DL_CreationInterface* creationInterface) {
    DL_Ellipse2dData d(toReal(values[10]),
                     toReal(values[20]),
                     toReal(values[30]),
                     toReal(values[11]),
                     toReal(values[21]),
    ...
    creationInterface->addEllipse2d(d);
}
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeEllipse2d(DL_WriterA& dw,
                          const DL_Ellipse2dData& data,
                          const DL_Attributes& attrib) {
    ...
    if (version>VER_R12) {
        ...
        if (version==VER_2000) {
            dw.dxfString(100, "AcDbEntity");
            dw.dxfString(100, "AcDbEllipse2d");
        }
        dw.entityAttributes(attrib);
        dw.coord(10, data.cx, data.cy);
